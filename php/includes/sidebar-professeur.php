<?php
/**
 * KLARO - Sidebar Professeur
 */
$current_page = basename($_SERVER['PHP_SELF']);

$nav_items = [
    ['url' => 'dashboard-professeur.php', 'label' => 'Dashboard', 'icon' => 'layout-dashboard'],
    ['url' => 'creer-quiz.php', 'label' => 'Créer un quiz', 'icon' => 'plus-circle'],
    ['url' => 'classrooms-professeur.php', 'label' => 'Mes Classrooms', 'icon' => 'graduation-cap'],
    ['url' => 'etudiants-professeur.php', 'label' => 'Étudiants', 'icon' => 'users'],
    ['url' => 'compositions-professeur.php', 'label' => 'Compositions créées', 'icon' => 'file-text'],
    ['url' => 'outils-ia.php', 'label' => 'Outils IA (Klaro AI)', 'icon' => 'sparkles'],
    ['url' => 'support.php', 'label' => 'Support', 'icon' => 'life-buoy'],
...
<aside class="w-full lg:w-64 bg-white rounded-2xl p-4 shadow-sm border border-slate-200/80 flex flex-col justify-between shrink-0 mb-6 lg:mb-0">
    <div>
        <!-- Profil rapide -->
        <div class="p-3 mb-4 rounded-xl bg-blue-50 border border-blue-100 flex items-center gap-3">
            <div class="w-10 h-10 rounded-full bg-blue-600 text-white font-bold flex items-center justify-center text-sm shadow-sm">
                <?= strtoupper(substr($_SESSION['prenom'] ?? 'P', 0, 1) . substr($_SESSION['nom'] ?? 'R', 0, 1)) ?>
            </div>
            <div class="overflow-hidden">
                <p class="text-sm font-bold text-slate-800 truncate"><?= e($_SESSION['prenom'] ?? '') ?> <?= e($_SESSION['nom'] ?? '') ?></p>
                <p class="text-xs font-semibold text-blue-600 uppercase tracking-wider">Professeur</p>
            </div>
        </div>
